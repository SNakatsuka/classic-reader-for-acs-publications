(() => {
  ...
  function isSupportedPage() {
    ...
    return path.endsWith('/latest-articles') || path.includes('/search-results') || path.includes('/article/doi/');
  }
  ...
  function findVisualElement(root) {
    ...
    if (visual && visual !== root && visual.querySelector('img')) return visual;

    const image = root.querySelector('img[alt*="visual abstract" i], img[alt*="graphical abstract" i]');
    if (!image) return null;
    return image.closest('.featured-img-wrapper, figure, [class*="visual"], [class*="graphic"]') || image.parentElement;
  }
  ...
  function placeListVisual(box) {
    ...
    if (existing) {
      if (visual && visual !== existing && !existing.contains(visual)) {
        visual.classList.add('acs-classic-visual-source');
      }
      return;
    }
    if (!visual) return;
    const marker = document.createComment('acs-classic-list-visual-original-position');
    visual.before(marker);
    const side = document.createElement('aside');
    side.className = 'acs-classic-list-visual acs-classic-native-visual';
    const title = document.createElement('div');
    title.textContent = 'Visual Abstract';
    side.append(title, visual);
    box.append(side);
    visual.classList.add('acs-classic-visual-source');
    box.classList.add('acs-classic-list-with-visual');
    relocatedVisuals.push({ intro: side, abstract: null, visual, marker, listBox: box });
  }
  ...
  function expandArticleList() {
    const boxes = $all('.al-article-box:not(.sr-list)');
    ...
    enableVisualFirst(boxes.filter((box) => !box.querySelector(':scope > .acs-classic-list-visual')));
  }

  function enhanceSearchResults() {
    document.documentElement.classList.add('acs-classic-search-page');
    const boxes = $all('.sr-list.content-type-journal-articles');
    if (!boxes.length) return;
    boxes.forEach((box) => {
      box.classList.add('acs-classic-search-card');
      placeListVisual(box);
    });
    enableVisualFirst(boxes.filter((box) => !box.querySelector(':scope > .acs-classic-list-visual')));
  }
  ...
  function apply() {
    if (!isSupportedPage()) { remove(); return; }
    document.documentElement.classList.add(ROOT);
    hideNoise();
    if (isSearchPage()) {
      enhanceSearchResults();
    } else {
      promoteAbstract(); placeVisualAbstract(); expandArticleList(); emphasizePdf();
    }
  }
  function remove() {
    document.documentElement.classList.remove(ROOT, 'acs-classic-search-page');
    ...
    $all('[data-acs-classic-visual-queued]').forEach((box) => delete box.dataset.acsClassicVisualQueued);
    $all('.acs-classic-list-card').forEach((box) => box.classList.remove('acs-classic-list-card'));
    $all('.acs-classic-search-card').forEach((box) => box.classList.remove('acs-classic-search-card'));
